            ...
            if (gi.sprite == "RedTile.PNG") {
                ...
                for (shot = 0; shot < currentProjectile.shots; shot++) {
                    if (randomint(0, 100) < currentProjectile.accuracy) {
                        ...
                        CreateConsoleText("Armor Chomper has hit "+gi.character.name+" for "+damagedone+" damage.",true);
                        ...
                        if (randomint(0, 100) < currentProjectile.stunChance) {
                            CreateConsoleText("Armor Chomper has stunned "+gi.character.name+" for one turn.") /*add visual change to indicate goop*/
...
Gun.shots = 2;
Gun.accuracy = 55;
Suitcase = new AttackType();
Suitcase.name = "Secret Suitcase";
Suitcase.damage = 50;
Suitcase.range = 1;
Suitcase.reloadTime = 3;
...
GunZomb.health = 100;
GunZomb.height = "28%";
GunZomb.powerLevel = 6;
GunZomb.attacks.push(Suitcase,Gun);
...
IsPlayerTurn = true;
CanMove = true;
CanAbility = true; /*let chomper do 2 abilityes*/
StopTurn = false;
directionright = true;
prevppos = AC.coords.slice(0);
                    ...
                    if (randomint(0, 100) < attack.accuracy) {
                        ...
                        if (!(CheckForLoss())) {
                            if (randomint(0, 100) < attack.stunChance) {
                                CreateConsoleText(zombie.name+" has stunned Armor Chomper for one turn.") /*add visual change to indicate AC is frozen*/
                                AC.stunned = true;
                            }
                        }
                        else {
